...
import api from '../api';
import { User, Mail, Lock, ArrowRight, Loader2, Plane } from 'lucide-react';
    ...
    <div className="relative min-h-[85vh] flex items-center justify-center">
      ...
      >
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-display font-bold mb-2 text-eggplant">Join Traveloop</h1>
          <p className="text-eggplant-muted font-medium">Start planning your dream journey today.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-2">
            <label className="text-[11px] font-bold text-eggplant-muted ml-1 uppercase tracking-widest">Full Name</label>
            <div className="relative">
              <User className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-eggplant/40" />
              <input 
                type="text" 
                required
                className="input-field w-full pl-12 font-medium"
                placeholder="Kavya Suresh"
                value={formData.name}
          ...
          <div className="space-y-2">
            <label className="text-[11px] font-bold text-eggplant-muted ml-1 uppercase tracking-widest">Password</label>
            <div className="relative">
        ...
        <div className="mt-8 text-center text-sm text-eggplant-muted font-medium border-t border-eggplant/5 pt-6">
          Already have an account? <Link to="/login" className="text-salmon font-bold hover:underline">Log in</Link>
        </div>
      </motion.div>
    </div>
  );
